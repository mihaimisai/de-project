from src.ingest.function.utils.upload_time_stamp import upload_time_stamp
...
class TestUploadTimeStamp:
    ...
    def test_upload_timestamp_success_info_logged(self, test_logger):
        ...
        with LogCapture(level=logging.INFO) as logstream:
            logstream.clear()
            upload_time_stamp(
                test_client, bucket_name, table_name, test_logger, time_stamp
            )
        ...
        for log in logstream:
            assert log == (
                "test_logger",
                "INFO",
                "Successfully uploaded time_stamp_test-users.txt file to S3 bucket 'Test_bucket'",  # noqa
            )
