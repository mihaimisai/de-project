import pytest
import pandas as pd
from src.transformation.function.utils.transform import (
...
class TestTransform:
    def test_transform_fact_sales_order(self):
        ...
        assert df_fact_sales_order["created_date"].tolist() == [pd.to_datetime("2025-03-03").date()] * 2
        assert df_fact_sales_order["last_updated_date"].tolist() == [pd.to_datetime("2025-03-03").date()] * 2

        # Verify time extraction
        assert df_fact_sales_order["created_time"].tolist() == [pd.to_datetime("10:22:01").time(), pd.to_datetime("11:45:30").time()]
        assert df_fact_sales_order["last_updated_time"].tolist() == [pd.to_datetime("10:30:00").time(), pd.to_datetime("12:00:00").time()]

        # Verify numeric values
        assert df_fact_sales_order["unit_price"].tolist() == [100.12, 200.57]  # Rounded values

        # Verify agreed dates
        assert df_fact_sales_order["agreed_payment_date"].tolist() == [pd.to_datetime("2025-03-05").date(), pd.to_datetime("2025-03-06").date()]
        assert df_fact_sales_order["agreed_delivery_date"].tolist() == [pd.to_datetime("2025-03-10").date(), pd.to_datetime("2025-03-11").date()]
    ...
    def test_transform_dim_staff(self):
        ...
        df_staff = pd.DataFrame({
            ...
            "department_id": [101, 102, 103]  # Note: department_id 103 won't have a match
        })
        ...
        assert list(df_dim_staff.columns) == expected_columns, "Output columns do not match expected columns."
        ...
        assert row_alice["department_name"] == "Sales", "Alice's department_name is incorrect."
        assert row_alice["location"] == "New York", "Alice's location is incorrect."
        assert row_alice["email_address"] == "sales@example.com", "Alice's email_address is incorrect."

        row_bob = df_dim_staff[df_dim_staff["staff_id"] == 2].iloc[0]
        assert row_bob["department_name"] == "Engineering", "Bob's department_name is incorrect."
        assert row_bob["location"] == "San Francisco", "Bob's location is incorrect."
        assert row_bob["email_address"] == "eng@example.com", "Bob's email_address is incorrect."

        # For staff with no matching department (e.g. department_id 103), the department fields should be NaN.
        row_charlie = df_dim_staff[df_dim_staff["staff_id"] == 3].iloc[0]
        assert pd.isna(row_charlie["department_name"]), "Expected NaN for Charlie's department_name."
        assert pd.isna(row_charlie["location"]), "Expected NaN for Charlie's location."
        assert pd.isna(row_charlie["email_address"]), "Expected NaN for Charlie's email_address."

    def test_transform_dim_location(self):
        # Create sample input data including an extra column to verify that only required columns are kept.
        data = {
            "address_id": [1, 2, 3],
            ...
            "extra": ["ignore", "this", "column"]  # Extra column that should not appear in the output.
        }
        ...
        # Verify the output DataFrame has the correct columns in the right order
        assert list(df_dim_location.columns) == expected_columns, "Output columns do not match expected columns."
        
        # Verify that the renaming was done correctly: "location_id" should match the original "address_id"
        expected_location_ids = df_address["address_id"].tolist()
        assert df_dim_location["location_id"].tolist() == expected_location_ids, "location_id values do not match address_id values."
        
        # Verify that other columns are correctly transferred
        for col in expected_columns[1:]:
            assert df_dim_location[col].tolist() == df_address[col].tolist(), f"Column {col} does not match the expected values."
        
        # Confirm that the extra column is not present in the output
        assert "extra" not in df_dim_location.columns, "Extra column found in output DataFrame."
    ...
    def test_transform_dim_design(self):
        ...
        data = {
            ...
            "extra_col": ["ignore1", "ignore2", "ignore3"]  # Extra column that should be dropped
        }
        ...
        # Check that output has exactly the expected columns in the correct order
        assert list(df_dim_design.columns) == expected_columns, "Output columns do not match expected columns."

        # Verify that the data for each column is preserved
        for col in expected_columns:
            assert df_dim_design[col].tolist() == df_design[col].tolist(), f"Values for column '{col}' do not match."

        # Confirm that the extra column is not present in the output
        assert "extra_col" not in df_dim_design.columns, "Unexpected extra column found in output DataFrame."
    ...
    def test_transform_dim_date(self):
        ...
        # Check that the DataFrame has exactly the expected columns in the right order
        assert list(df_dim_date.columns) == expected_columns, "Output columns do not match expected columns."
        ...
        assert df_dim_date.iloc[0]["date_id"] == first_date, "The first date is not 2020-01-01."
        assert df_dim_date.iloc[-1]["date_id"] == last_date, "The last date is not 2125-12-31."
        
        # Check that the total number of rows equals (last_date - first_date).days + 1
        expected_length = (last_date - first_date).days + 1
        assert len(df_dim_date) == expected_length, f"Expected {expected_length} rows, but got {len(df_dim_date)}."
        ...
        assert first_row["year"] == 2020, "Year for the first row is incorrect."
        assert first_row["month"] == 1, "Month for the first row is incorrect."
        assert first_row["day"] == 1, "Day for the first row is incorrect."
        assert first_row["day_of_week"] == 3, "Day of week for 2020-01-01 should be 3 (Wednesday)."
        assert first_row["day_name"] == "Wednesday", "Day name for 2020-01-01 should be 'Wednesday'."
        assert first_row["month_name"] == "January", "Month name for the first row is incorrect."
        ...
        assert sample_row["day_of_week"] == 5, "Day of week for 2021-12-31 should be 5 (Friday)."
        assert sample_row["day_name"] == "Friday", "Day name for 2021-12-31 should be 'Friday'."
        assert sample_row["month_name"] == "December", "Month name for 2021-12-31 is incorrect."
        assert sample_row["quarter"] == 4, "Quarter for 2021-12-31 should be 4."

    def test_transform_dim_currency(self):
        # Create sample input data, including an extra column that should be dropped.
        data = {
            "currency_id": [1, 2, 3, 4],
            "currency_code": ["USD", "EUR", "ABC", "JPY"],
            "extra_col": ["ignore1", "ignore2", "ignore3", "ignore4"]  # Extra column that should be ignored.
        }
        ...
        # Define the expected columns and check that the output DataFrame has exactly these columns in order.
        expected_columns = ["currency_id", "currency_code", "currency_name"]
        assert list(result.columns) == expected_columns, "Output columns do not match expected columns."
        ...
        for i, expected_name in enumerate(expected_currency_names):
            ...
            if expected_name is None:
                ...
                assert pd.isna(actual_name), f"Expected NaN for currency_code 'ABC', got {actual_name}"
            else:
                assert actual_name == expected_name, f"Row {i}: expected '{expected_name}', got '{actual_name}'"
    ...
    def test_transform_dim_counterparty(self):
        ...
        # Check that the output DataFrame has exactly the expected columns in order
        assert list(result.columns) == expected_columns, "Output columns do not match expected columns."
