...
class TestUploadTimeStamp:
    ...
    def test_upload_timestamp_success_info_logged(self,
                                                  test_logger):
        ...
        with LogCapture(level=logging.INFO) as logstream:
            logstream.clear()
            upload_time_stamp(test_client, bucket_name,
                              table_name, test_logger)
    ...
    @mock_aws
    @freeze_time("2025-01-01 01:00:00")
    def test_upload_timestamp_error_logged_when_no_bucket(self,
                                                          test_logger):
        test_client = boto3.client("s3")
        bucket_name = "Test_bucket"
        table_name = "test-users"
