...
class TestUploadTimeStamp:
    ...
    def test_upload_timestamp_success_info_message_logged(self,
                                                          test_logger):
        ...
        for log in logstream:
            assert log == ('test_logger', 'INFO', "Successfully uploaded 2025-01-01 01:00:00_test-users.txt file to S3 bucket 'Test_bucket'")

    @mock_aws
    @freeze_time("2025-01-01 01:00:00")
    def test_upload_timestamp_error_message_logged_when_no_bucket(self,
                                                          test_logger):
        test_client = boto3.client("s3")
        bucket_name = "Test_bucket"
        table_name = "test-users"
        ...
        for log in logstream:
            assert log == ('test_logger', 'ERROR', "Error uploading time_stamp_test-users.txt to S3 bucket: 'Test_bucket': An error occurred (NoSuchBucket) when calling the PutObject operation: The specified bucket does not exist")
    ...
    @mock_aws
    @freeze_time("2025-01-01 01:00:00")
    def test_upload_timestamp_to_bucket_success(self,
                                                test_logger):
        test_client = boto3.client("s3")
        bucket_name = "Test_bucket"
        table_name = "test-users"
