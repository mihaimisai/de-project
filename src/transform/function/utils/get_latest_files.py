def get_latest_files(client, bucket, logger):
    ...
    try:
        ...
        list_objects = [obj['Key'] for obj in sorted(objs.get('Contents', []), key=get_last_modified)]
        ...
        logger.info(
            f"Succesfully retrieved files from bucket:{bucket}"
        )
        ...
        logger.error(
            f"Error getting files from bucket:{bucket}: {e}"
        )
        raise Exception(e)