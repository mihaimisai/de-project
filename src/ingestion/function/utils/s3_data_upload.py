...
def s3_data_upload(client, bucket_name, table_name, csv_df, logger):
    ...
    Uploads csv file containing ingested data to s3 ingested data bucket.
        ...
        Parameters:
            ...
            table_name (str): table data has been retrieved from
                - referenced in s3 file name
            csv_df (csv): csv file to upload
            logger (Logger): logger instance

        Logs:
            either info f"Successfully uploaded csv file
            to S3 bucket '{bucket_name}' for table '{table_name}'"
            if successful

            or error f"Error uploading csv file to
            S3 for table '{table_name}': {e}"
    ...
    try:
        ...
        s3_key_ingestion = (
            f"{table_name}/{year}/{month}/{day}/{time_stamp}.csv"  # noqa
        )
